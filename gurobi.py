    ...
    for node in G.nodes():
        X[node] = m.addVar(vtype=GRB.BINARY, name="X_{}".format(node) )
    ...
    for x in G.nodes():
        for y in G.nodes():
            if(Edges.has_key((x,y)) == False and Edges.has_key((y,x)) == False):
                print("Add {}{}".format(x,y))
                Edges[x,y] =  m.addVar(vtype=GRB.BINARY, name="X_{}_{}".format(x,y))

                print("Add {}{}".format(y,x))
                Edges[y,x] =  m.addVar(vtype=GRB.BINARY, name="X_{}_{}".format(y,x))
            if(Arrete.has_key((x,y)) == False and Arrete.has_key((y,x)) == False):
                #print("Add {}{}".format(x,y))
                Arrete[x,y] =  0
                #print("Add {}{}".format(y,x))
                Arrete[y,x] =  0
    ...
    m.setObjective(quicksum(Edges[(x,y)]*Arrete[(x,y)] for (x,y) in Arrete.keys()),GRB.MAXIMIZE)
        ...
        for j in G.nodes():
            for k in G.nodes():
                if((i != j) and (j != k) and (i != k)):
                    ...
                    #m.addConstr((Xij + Xik + Xjk == 1) or (Xij + Xik + Xjk == 0) or (Xij + Xik + Xjk) == 3)
